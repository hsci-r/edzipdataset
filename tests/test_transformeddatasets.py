...
from hscitorchutil.dataset import KeyTransformingMapDataset, EntryTransformingMapDataset
import pickle
...
class TestTransformedMapDataset(unittest.TestCase):
    ...
    def test_transformedmapdataset(self):
        ds = EntryTransformingMapDataset(
            self.dataset, lambda l: [x[0]+2 for x in l])
    ...
    def test_transformeddataset_pickling(self):
        ds = EntryTransformingMapDataset(self.dataset, _transform)
        self.assertEqual(len(ds), 3)
        self.assertEqual(ds[0], 2)
        self.assertEqual(ds[1], 3)
        self.assertEqual(ds[2], 4)
    ...
    def test_transformeddataset_with_args_pickling(self):
        ds = KeyTransformingMapDataset(EntryTransformingMapDataset(
