...
def dbname(tmp_path_factory) -> str:
    ...
    with con:
        con.execute("CREATE TABLE test (entry_number INTEGER PRIMARY KEY, id VARCHAR(255), value INTEGER)")
...
def test_sqlitedatamodule(dbname: str, tmp_path):
    ...
    db = SQLiteDataModule(dbname, dbname, dbname, str(tmp_path / "cache"), "test", "entry_number", "value, id", "id")
    db.prepare_data()
    db.setup()
    assert len(db.train_dataloader()) == 1
    vals, ids = db.train_dataloader().__iter__().__next__()
    assert len(vals) == 3
    assert len(ids) == 3
    assert len(db.val_dataloader()) == 1 # type: ignore
    assert len(db.test_dataloader()) == 1 # type: ignore
